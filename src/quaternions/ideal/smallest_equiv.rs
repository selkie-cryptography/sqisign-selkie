...
impl LeftIdeal<4> {
    ...
    pub fn smallest_equiv_with_delta(&self) -> Option<(Self, Element<4>)> {
        ...
        // reason: the body writes new_cols[j] and reads lattice.basis_elem(j)
        // in parallel; both are indexed by j, but lattice has no
        // iterator over its basis elements, so iter_mut().enumerate()
