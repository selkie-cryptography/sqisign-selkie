...
impl<const N: usize> NrdBasis<N> {
    ...
    pub fn l2_reduce(mut self) -> Self {
        ...
        while k < D {
            ...
            // Deep insertion (C ref's `quat_lll_core` ordering, l2.c:110):
            // iterate s from k down to 1, BREAK on first level
            // where Lovász holds (i.e., δ̄ · r[s-1][s-1] >= t[s-1]).
            // Resulting `s` is the deepest level such that Lovász
            // fails at all levels in (s, k]. In monotonic cases
            // this equals "smallest j where t[j] < δ̄·r[j][j]"; under DPE
            // rounding a non-monotonic pattern can occur and the
            // iteration order chosen here matches C ref's
            // byte-for-byte.
