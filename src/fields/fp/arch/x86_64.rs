...
//!
//! - `mulx_adx` -- radix-2^64 `Fp64` packed into `[u64; 4]`. Designed for MULX
//!   + dual-chain ADCX/ADOX scalar asm; matches the C ref's
//!   `gf/broadwell/lvl1/gf5248.c` storage exactly.  Library code only -- the
//!   dispatcher in `super::super` does NOT activate Fp64 as the crate-wide `Fp`
//!   yet; higher-level isogeny tests surfaced correctness issues that need
//!   debugging on x86_64 before the dispatcher can flip.  Module is cfg-gated
//!   on `target_feature = "bmi2"` + `target_feature = "adx"`; not in scope on
//!   doc builds without them.
//! - [`avx2`] -- radix-2^26 `Fp26x4` 4-wide batch, backed by AVX2
//!   `_mm256_mul_epu32`.  Not a standalone active `Fp` backend; exposed as a
//!   batch helper for call sites that explicitly opt into 4-Fp-at-once storage.
