...
// The `sum_of_N_products` / `difference_of_N_products` family below is
// hand-unrolled per N (one explicit method body per supported t-value
// in Longa Alg 5).  An earlier experiment (`sum-of-products-const-generic`
// branch) folded all the bodies into a single
// `pub fn sum_of_products_generic<const N: usize>(pairs: [(&Fp, &Fp); N])
// -> Fp` over `for k in 0..9 { for &(a, b) in pairs.iter() { ... } }`.
// On M4 with rustc 1.95.0 / opt-level 3 the generic measured 2.88x
// slower than the hand-unrolled longhand at N=4 (61.61 ns vs 21.40 ns)
// and 1.33x slower than the naive 6-mul-and-add baseline at N=6 (89.59 ns
// vs 67.45 ns) — LLVM didn't unroll the nested loops despite N being a
// compile-time constant, leaving runtime branches in the hot path and
// destroying the fused-reduction win.  The handwritten bodies map one
// column per source paragraph and are auditable line-by-line; they pay
// in LOC (~200 / method) for guaranteed codegen.  If a future rustc /
// optimiser improvement closes the gap, the generic body is preserved
// on the `sum-of-products-const-generic` branch as a reference oracle.
...
impl Fp {
    ...
    /// would each carry their own reduction.
    ///
    /// # Implementation
    ///
    ...
    /// Montgomery reduction.
    ///
    /// The t=6 extension of [`Fp::sum_of_2_products`].  Used by
    ...
    #[must_use]
    #[rustfmt::skip]
    pub fn sum_of_6_products(pairs: [(&Fp, &Fp); 6]) -> Fp {
    ...
    ///
    /// Negates the last pair's `b` and defers to
    /// [`Fp::sum_of_6_products`].  Multi-negation patterns (e.g. the
    ...
    #[must_use]
    pub fn difference_of_6_products(pairs: [(&Fp, &Fp); 6]) -> Fp {
        let [p1, p2, p3, p4, p5, (a6, b6)] = pairs;
        let neg_b6 = -b6;
        Fp::sum_of_6_products([p1, p2, p3, p4, p5, (a6, &neg_b6)])
    }
}
