...
pub(crate) struct TwoIsogeny {
    ...
    /// Kernel generator on the domain, an order-2 `x`-only point.
    kernel: ProjectiveXOnlyPoint,
}

impl TwoIsogeny {
    /// Builds the 2-isogeny with kernel generated by `P`.
...
pub(crate) struct TwoIsogenySingular {
    /// Codomain curve `E'`.
    pub(crate) codomain: Curve,
    /// Evaluation constant matching C ref's `K.z = −√(A² − 4)`.
    c0: Fp2,
    /// Evaluation constant matching C ref's `K.x = A`.
    c1: Fp2,
}

impl TwoIsogenySingular {
    /// Builds the singular 2-isogeny with kernel `(0, 0)` on `curve`.
...
pub(crate) struct FourIsogeny {
    /// Codomain curve `E'`.
    pub(crate) codomain: Curve,
    /// First evaluation constant precomputed from the kernel.
    c0: Fp2,
    /// Second evaluation constant precomputed from the kernel.
    c1: Fp2,
    /// Third evaluation constant precomputed from the kernel.
    c2: Fp2,
}

impl FourIsogeny {
    /// Builds the 4-isogeny with kernel generated by `P`.
