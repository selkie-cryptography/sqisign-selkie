...
optimisation and constant-time hardening built on top of the
...
$64$~limbs at the smaller auxiliary one (branch
\texttt{mlll}).  Cost scales $O(W^2)$, so the narrowing is large.
Deterministic measurement: $-11.8\%$ on \texttt{kat\_sign}, output
byte-identical.  A follow-up narrowed the widths further to the
$\leq 110$-limb bound proven by Kim et al.\ (ePrint 2025/1649),
giving $110$~limbs at the main site and $48$ at the auxiliary site
(branch \texttt{narrower-mlll-widths}).
...
\texttt{compute\_r\_squared} are now branchless.  Output is identical.
Deterministic measurement (branch \texttt{ct-modexp}): $-1.01\%$ on
\texttt{kat\_keygen}, $-0.10\%$ on \texttt{kat\_sign}.  Verify
untouched.

\paragraph{Windowed Pohlig--Hellman discrete logarithm.}
\texttt{RootOfUnity::dlog} on the 2-power pairing-output group
$\mu_{2^e}$ was rewritten from a recursive halving Pohlig--Hellman
into an iterative windowed variant with window width $W = 4$ and a
constant-time table scan.  The old recursion called \texttt{pow\_scalar}
at every node, walking $256$ bits regardless of the exponent size, and
was variable-time on the secret-derived bases $M_{\mathrm{sk}}$ and
$M_{\mathrm{chl}}$; that leak is closed.  Deterministic measurement
(branch \texttt{windowed-ph-dlog}): $-4.84\%$ on \texttt{kat\_keygen},
$-1.33\%$ on \texttt{kat\_sign}, $-2.03\%$ total --- the largest
single-branch improvement so far.  Verify essentially unchanged
($-0.04\%$).

\paragraph{Cost-optimal isogeny strategy for the 4-isogeny chain.}
The verify-side $2^e$-isogeny chain, decomposed into
$\lfloor e/2 \rfloor$ 4-isogenies, was changed from runtime balanced
halving to a compile-time-precomputed cost-optimal De Feo--Jao--Plut
strategy (standard SIDH dynamic programming on doubling cost $p$ and
isogeny pushforward cost $q$).  Output is byte-identical for the
4-isogeny chain (branch \texttt{optimal-isogeny-strategy}).

\subsection{Tried approaches that did not land}
\label{sec:tried-rejected}

Each entry below names the branch it was attempted on, what was tried,
and the reason it did not land.

\paragraph{Cost-optimal strategy on the $(2,2)$-theta chain (surfaces).}
The dynamic-programming strategy that worked on the verify-side
4-isogeny chain was applied to Phase~1 of the $(2,2)$-theta chain in
\texttt{src/surfaces/mod.rs}.  $166$ of $707$ lib tests failed,
...
single inversion via Montgomery's trick.  Output is identical and every
KAT byte-identical.  However deterministic measurement (branch
\texttt{batch-inversion-audit}) showed a $+0.05\%$ regression overall
and $+0.62\%$ on \texttt{kat\_verify}: the two-pass refactor's overhead
...
\texttt{kat\_keygen} in a controlled A/B.

\paragraph{Input-LLL preprocessing of the intersection.}
The input-LLL preprocessing of~\cite{KLY26} reduces each input basis
before dualisation, intending to bound the duals' cubic coefficient
growth.  Our existing \texttt{l2\_reduce} routine minimises the
...
\begin{enumerate}
  \item Hanyecz et al.\ Euclidean dim-4 LLL (ePrint 2025/027) as
    preprocessing in front of the MLLL intersection, to push the
    working width below the $\leq 110$-limb proven bound toward the
    $\sim 28$-limb compact bound of~\cite{KLY26}.
  ...
  \item Continue the constant-time hardening sequence
    (\S\ref{sec:ct}).  Remaining variable-time surface lives in
    \texttt{MontReducer::pow}'s base reduction (\texttt{mag\_div\_rem}),
    in \texttt{l2\_reduce} on secret-derived input, and elsewhere in
    the quaternion-arithmetic layer.
\end{enumerate}
