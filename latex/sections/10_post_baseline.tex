...
block encryption, and the key-schedule cadence is fixed by the
CTR-DRBG \texttt{update} cycle, so it is not byte-identically
recoverable.  The DRBG was switched from the full \texttt{aes::Aes256}
cipher to the encryption-only \texttt{Aes256Enc}, since CTR mode never
decrypts; this drops the wasted decryption key schedule and is the
correct cipher type for the construction.
