...
\cdot 19 \cdot 23 \cdot 29$ before any exponentiation.  We add the same
step, recovering each residue $n \bmod p$ from a single reduction of $n$
modulo that product.  A candidate caught by the pre-screen is composite
and the witness exponentiation would reject it too, so the accept/reject
decision, and every downstream signature, is unchanged; matching the
reference's prime set exactly makes this a step the reference already
performs rather than an independent argument about which composites the
exponentiation rejects.  Output byte-identical (all KATs green).
Instruction-count A/B (Apple M4): $-6.4\%$ on key generation, $-4.6\%$
on signing.  Key generation gains the most because its prime search
issues far more candidate tests per operation than signing does.
