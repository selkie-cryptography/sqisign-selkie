...
Deterministic measurement: $-11.8\%$ on signing, output
...
key generation, $-0.10\%$ on signing.  Verification
untouched.
...
(branch \texttt{windowed-ph-dlog}): $-4.84\%$ on key generation,
$-1.33\%$ on signing, $-2.03\%$ total --- the largest
single-branch improvement so far.  Verification essentially unchanged
($-0.04\%$).
...
Feo--Jao--Plut strategy.  Output is byte-identical (branch
\texttt{optimal-isogeny-strategy}).  Deterministic measurement:
...
and $+0.62\%$ on verification: the two-pass refactor's overhead
