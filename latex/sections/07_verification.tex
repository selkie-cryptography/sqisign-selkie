...
\begin{itemize}[nosep]
  \item KAT correctness (Layer~7): \textbf{partially done}.
        \emph{Verify} accepts 100/100 C ref signatures; \emph{sign}
        completes for ${\geq}~50$/100 KAT seeds within the current
        CI per-test budget.  All completed signatures
        cross-verify against the C ref harness.  The remaining
        gap is sign throughput, not correctness.
  \item Empirical CT (Layer~5): \textbf{infrastructure present, not
        a verification claim}.  \texttt{dudect} and \texttt{tacet}
        runs are wired into CI; baseline measurements exist but
        the codebase is not yet constant-time on secret-derived
