...
\finding{Affects \algo{8.47} (\textsc{Isogeny22ChainWithTorsion})
  and its callers, primarily \algo{3.13} line~9.}{\low}

The spec describes a single uniform chain of $e$ 8-torsion
isogenies with a kernel of order~$2^{e+2}$.  The C reference
implementation in \texttt{theta\_isogenies.c} splits this into two
variants, keyed on a boolean \texttt{extra\_torsion}:
\begin{itemize}
  \item \texttt{extra\_torsion = true}: as in the spec, an
    $e$-length chain of 8-torsion isogenies.
  \item \texttt{extra\_torsion = false}: an $(e-2)$-length chain
    of 8-torsion isogenies followed by a 4-isogeny and a
    2-isogeny step.  This consumes a kernel of order~$2^e$ rather
    than~$2^{e+2}$, and the final two steps use square roots.
\end{itemize}
\algo{3.13} line~9 calls the outer chain with
\texttt{extra\_torsion = false} (\texttt{dim2id2iso.c:1128})
whereas the inner \algo{3.15} calls it with
\texttt{extra\_torsion = true} (\texttt{dim2id2iso.c:181}).  The
two variants produce the same $(2^e, 2^e)$-isogeny but require
different kernel torsions and different implementation paths.

\paragraph{Impact.}
Implementations that only supply the $e$-length 8-torsion variant
must pad the kernel to order~$2^{e+2}$, which is impossible when
$e$ is close to the target curve's available 2-power torsion~$f$.
In particular, for $e \in \{f-1, f\}$ the padded kernel does not
exist.  The C reference handles this by falling back to the
shorter chain with 4-iso/2-iso tails, but the spec gives no
guidance.

\paragraph{Recommendation.}
Either describe both variants in \algo{8.47} with a note on when
each is required, or specify the 4-iso/2-iso tail as part of the
algorithm so that implementations can support the full range of
$e \le f$ without ad-hoc workarounds.
