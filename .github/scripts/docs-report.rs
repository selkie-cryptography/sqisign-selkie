//! Run `cargo doc` and count warnings. Output JSON.
//!
//! Usage: docs-report <sha>
//!
//! Compile: `rustc -O docs-report.rs -o docs-report`
...
fn main() -> io::Result<()> {
    ...
    if args.len() != 2 { eprintln!("usage: docs-report <sha>"); std::process::exit(1); }
    let sha = &args[1];

    let output = Command::new("cargo").args(["doc", "--no-deps"])
        .stdout(Stdio::piped()).stderr(Stdio::piped()).output()
        .expect("failed to run cargo doc");
    let text = String::from_utf8_lossy(&output.stderr);
    let warnings = text.lines().filter(|l| l.contains("warning")).count();
    let status = if warnings == 0 { "pass" } else { "warn" };
